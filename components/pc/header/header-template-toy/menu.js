...
class MenuGroup extends Component {
    ...
    render() {
        ...
        return (
            <div className={this.props.atTop ? 'nav-wrapper nav-wrapper-2  nav-wrapper-3 nav-wrapper-6' : 'nav-wrapper nav-wrapper-2  nav-wrapper-3 nav-wrapper-6 nav-wrapper-common'}>
                <div className="nav-menu"  style={this.state.showSearch ? { opacity: 0} : {width:'78%', opacity: 1, marginLeft: '5%'}} >
                ...
                <SubMenu title={
                    ...
                    {menuList.map((item, i) => {
                        if (item.subList.length) {  //有二级分类
                            return (<SubMenu key={item.categoryId} title={item.name} className={this.state.changeStyle ? 'menu-subMenu-item menu-subMenu-change': 'menu-subMenu-item'}>
                                {
